    ...
    <div class="flex-1 lg:pl-72">
        <div class="p-6 lg:p-10 max-w-4xl">
            ...
            <form action="{{ route('documents.store') }}" method="POST" enctype="multipart/form-data" id="uploadForm">
                ...
                <input type="hidden" name="cover_base64" id="cover_base64" value="">
                <!-- ============================================ -->
                ...
                <div class="space-y-6">
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-200/80 overflow-hidden">
                        ...
                        <div class="p-6">
                            ...
                            <!-- ============================================ -->
                            <!-- COVER PREVIEW - ADD THIS SECTION -->
                            <!-- ============================================ -->
                            <div id="coverPreviewSection" class="hidden mt-4 pt-4 border-t border-gray-100">
                                ...
                                <img 
                                    ...
                                    src="" 
                                    alt="Cover preview" 
                                    class="w-28 h-40 object-cover rounded-lg shadow-md border border-gray-200"
                                >
                            </div>
                            <!-- ============================================ -->
                            
                            <!-- Cover generation status -->
                            <div id="coverGenerating" class="hidden mt-3 flex items-center gap-2 text-xs text-blue-600">
...
@push('scripts')
<!-- ============================================ -->
<!-- PDF.JS LIBRARY - THIS WAS MISSING! -->
<!-- ============================================ -->
...
document.addEventListener('DOMContentLoaded', function() {
    // ============================================
    // INITIALIZE PDF.JS - THIS WAS MISSING!
    // ============================================
    ...
    // ============================================
    // NEW: Cover-related elements
    // ============================================
    const coverBase64Input = document.getElementById('cover_base64');
    const coverPreviewSection = document.getElementById('coverPreviewSection');
    ...
    // ============================================
    // NEW: Generate cover from PDF
    // ============================================
    async function generateCoverFromPDF(file) {
        if (file.type !== 'application/pdf') {
            console.log('Not a PDF, skipping cover generation');
            return null;
        }

        try {
            coverGenerating.classList.remove('hidden');
            console.log('Starting PDF cover generation for:', file.name);
            ...
            console.log('Cover generated successfully, length:', base64.length);
            
            // Show preview
            coverPreview.src = base64;
            coverPreviewSection.classList.remove('hidden');
            
            // Set the hidden input value
            coverBase64Input.value = base64;
            
            coverGenerating.classList.add('hidden');
            ...
            coverPreviewSection.classList.add('hidden');
            return null;
        }
    }

    // ============================================
    // NEW: Generate cover from image file
    // ============================================
    function generateCoverFromImage(file) {
        ...
        return new Promise((resolve) => {
            ...
            reader.onload = (e) => {
                ...
                console.log('Image cover generated, length:', base64.length);
                resolve(base64);
            };
            reader.onerror = () => {
    ...
    async function handleFile(file) {
        if (file) {
            ...
            // ============================================
            // NEW: Generate cover based on file type
            // ============================================
            clearCover();
            ...
            if (file.type === 'application/pdf') {
                ...
                await generateCoverFromImage(file);
            }
            // For DOC/DOCX/TXT, no automatic cover generation
            // ============================================
        }
    }
    ...
    function resetFile() {
        ...
        // ============================================
        // NEW: Clear cover when file is removed
        // ============================================
        clearCover();
        // ============================================
    }
    ...
    form.addEventListener('submit', function() {
        // Debug: log if cover is being sent
        console.log('Submitting with cover_base64:', coverBase64Input.value ? 'YES (' + coverBase64Input.value.length + ' chars)' : 'NO');
