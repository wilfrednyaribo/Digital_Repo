<style>
    ...
    nav[class*="bg-white"][class*="border-b"] {
    ...
    .btn-upload.add-category:hover::after {
        left: 100%;
    }
...
.doc-card:hover{
  ...
  .doc-cover img {
    ...
    .doc-file-type-badge.pdf {
        background: rgba(220, 38, 38, 0.92);
        color: white;
    }

    .doc-file-type-badge.doc {
        background: rgba(37, 99, 235, 0.92);
        color: white;
    }

    .doc-file-type-badge.img {
        background: rgba(5, 150, 105, 0.92);
        color: white;
    }

    .doc-file-type-badge.default {
    ...
    .fake-cover.pdf {
        ...
        background: linear-gradient(160deg, #fef2f2 0%, #fff5f5 40%, #ffffff 100%);
    }

    .fake-cover.word {
        border-bottom-color: #3b82f6;
        background: linear-gradient(160deg, #eff6ff 0%, #f0f7ff 40%, #ffffff 100%);
    }
    ...
    .fake-cover.pdf .fake-cover-icon {
        background: linear-gradient(135deg, #ef4444, #dc2626);
        color: white;
    }

    .fake-cover.word .fake-cover-icon {
        background: linear-gradient(135deg, #3b82f6, #2563eb);
        color: white;
    }

    .fake-cover.default .fake-cover-icon {
    ...
    .cover-section-header .form-label {
        margin-bottom: 0;
    }
    ...
    .btn-cat-submit.loading .submit-spinner {
        display: inline-block;
    }

    .btn-cat-submit.loading .submit-text {
        display: none;
    }
    ...
    .cat-existing-tags::-webkit-scrollbar {
        width: 3px;
    }

    .cat-existing-tags::-webkit-scrollbar-thumb {
        background: #e2e8f0;
        border-radius: 2px;
    }
    ...
    .cat-existing-tag.newly-added .tag-dot {
        background: #f59e0b;
    }
    ...
    <div style="position: fixed; top: 0; left: 0; right: 0; bottom: 0; display: flex; z-index: 999;">
        ...
        <aside class="sidebar">
            ...
            <nav class="sidebar-nav">
                ...
                <a href="{{ route('dashboard') }}" class="nav-link @if (request()->routeIs('dashboard')) active @endif"><i
                        class="fas fa-gauge-high"></i><span>Dashboard</span></a>
                <div class="nav-section-label">Discover</div>
                <a href="{{ route('documents.index') }}"
                    class="nav-link @if (request()->routeIs('documents.index') && !request()->has('my_uploads')) active @endif"><i
                        class="fas fa-compass"></i><span>Browse All</span><span
                        class="nav-badge">{{ $documents->total() }}</span></a>
                <a href="{{ route('documents.manage') }}" class="nav-link"><i class="fas fa-table-list"></i> Manage
                    All</a>
                <a href="{{ route('documents.index', ['sort' => 'newest']) }}" class="nav-link"><i
                        class="fas fa-clock-rotate-left"></i><span>Recent Additions</span></a>
                <a href="{{ route('documents.index', ['filter' => 'popular']) }}" class="nav-link"><i
                        class="fas fa-fire"></i><span>Most Popular</span></a>
                <div class="nav-section-label">Repository</div>
                <div class="menu-group">
                    <div class="menu-header active" onclick="toggleMenu(this)"><span>By Type</span><i
                            class="fas fa-chevron-down menu-arrow"></i></div>
                    <div class="submenu open">
                        <a href="{{ route('documents.index', ['type' => 'book']) }}" class="nav-link"><i
                                class="fas fa-book"></i> Books & Monographs</a>
                        <a href="{{ route('documents.index', ['type' => 'record']) }}" class="nav-link"><i
                                class="fas fa-file-shield"></i> Archives & Records</a>
                        <a href="{{ route('documents.index', ['type' => 'file']) }}" class="nav-link"><i
                                class="fas fa-photo-film"></i> Media & Files</a>
                        <a href="{{ route('documents.index', ['type' => 'thesis']) }}" class="nav-link"><i
                                class="fas fa-graduation-cap"></i> Theses & Dissertations</a>
                    </div>
                </div>
                @auth
                    <div class="menu-group">
                        <div class="menu-header" onclick="toggleMenu(this)"><span>My Workspace</span><i
                                class="fas fa-chevron-down menu-arrow"></i></div>
                        <div class="submenu">
                            <a href="{{ route('documents.index', ['my_uploads' => auth()->id()]) }}" class="nav-link"><i
                                    class="fas fa-user-clock"></i> My Uploads</a>
                            <a href="#" class="nav-link"><i class="fas fa-heart"></i> Favorites</a>
                            <a href="#" class="nav-link"><i class="fas fa-clock-rotate-left"></i> Drafts <span
                                    class="nav-badge warning">2</span></a>
                        </div>
                        <div class="menu-group">
                            <div class="menu-header" onclick="toggleMenu(this)"><span>My Workspace</span><i
                                    class="fas fa-chevron-down menu-arrow"></i></div>
                            <div class="submenu">
                                <a href="{{ route('documents.index', ['my_uploads' => auth()->id()]) }}"
                                    class="nav-link"><i class="fas fa-user-clock"></i> My Uploads</a>
            ...
            @auth
                <div class="sidebar-footer">
                    <div class="user-card">
                        ...
                        <div class="user-info"><span class="user-name">{{ Auth::user()->name }}</span><span
                                class="user-role">Administrator</span></div>
                    </div>
                    <form method="POST" action="{{ route('logout') }}" style="margin-top: 0.5rem;">@csrf<button
                            type="submit" class="logout-link"><i class="fas fa-sign-out-alt"
                                style="margin-right: 6px;"></i> Sign Out</button></form>
                </div>
            @endauth
        </aside>
        ...
        <main class="main-content">
            <div class="page-header">
                <div class="page-header-left">
                    ...
                    <div class="header-date"><i class="fas fa-calendar-day"></i> {{ now()->format('l, F j, Y') }}
                    </div>
                </div>
                @auth
                    <div class="header-actions">
                        ...
                        <button onclick="openModal('file')" class="btn-upload file"><i class="fas fa-cloud-arrow-up"></i>
                            Upload Publication/Report</button>
                        <button onclick="openModal('book')" class="btn-upload book"><i class="fas fa-book-open"></i>
                            Upload Book</button>
                        <style>
                            .btn-manage-all {
            ...
            <div class="scroll-area">
                @if (session('success'))
                    <div class="success-alert">
                        <div class="success-alert-icon"><i class="fas fa-check"></i></div>
                        <p>{{ session('success') }}</p>
                    </div>
                @endif
                ...
                <div class="stats-grid">
                    <div class="stat-card blue">
                        <div class="stat-card-top">
                            <div class="stat-icon"><i class="fas fa-folder-open"></i></div><span
                                class="stat-trend neutral"><i class="fas fa-database"></i> All</span>
                        </div>
                        <div class="stat-label">Total Records</div>
                        <div class="stat-value">{{ number_format($documents->total()) }}</div>
                        <div class="stat-sub">Complete document inventory</div>
                    </div>
                    <div class="stat-card green">
                        <div class="stat-card-top">
                            <div class="stat-icon"><i class="fas fa-layer-group"></i></div><span
                                class="stat-trend up"><i class="fas fa-check-circle"></i> Active</span>
                        </div>
                        <div class="stat-label">Categories</div>
                        <div class="stat-value" id="statCategoryCount">{{ $categories->count() }}</div>
                        <div class="stat-sub">Organized classifications</div>
                    </div>
                    <div class="stat-card orange">
                        <div class="stat-card-top">
                            <div class="stat-icon"><i class="fas fa-clock"></i></div><span class="stat-trend up"><i
                                    class="fas fa-arrow-up"></i> New</span>
                        </div>
                        <div class="stat-label">Recent Uploads</div>
                        <div class="stat-value">{{ number_format($recentCount) }}</div>
                        <div class="stat-sub">Added in last 7 days</div>
                    </div>
                    <div class="stat-card purple">
                        <div class="stat-card-top">
                            <div class="stat-icon"><i class="fas fa-bolt"></i></div><span class="stat-trend up"><i
                                    class="fas fa-database"></i> Size</span>
                        </div>
                        <div class="stat-label">Total Size</div>
                        <div class="stat-value">{{ number_format($totalSize / 1048576, 1) }}<span
                                style="font-size:0.85rem;font-weight:600;"> MB</span></div>
                ...
                <div class="filter-bar">
                    ...
                    <div class="filter-controls">
                        <form id="filterForm" action="{{ route('documents.index') }}" method="GET"
                            ...
                            <div class="search-box">
                                ...
                                <input type="text" name="search" id="liveSearch"
                                    ...
                                    autocomplete="off">
                                <button type="button" class="btn-reset {{ request('search') ? 'visible' : '' }}"
                                    id="btnResetSearch" title="Clear search">
                ...
                <div class="doc-grid">
                    @forelse($documents as $doc)
                        <div class="doc-card">
                            <div class="doc-cover">
                                ...
                                @else
                                    <div
                                        class="fake-cover {{ $doc->file_type == 'pdf' ? 'pdf' : (in_array($doc->file_type, ['doc', 'docx']) ? 'word' : 'default') }}">
                                        ...
                                        <div class="fake-cover-subtitle">{{ strtoupper($doc->file_type ?? 'file') }}
                                            Document</div>
                                    </div>
                                @endif
                                ...
                                @php
                                    ...
                                    if ($doc->file_type == 'pdf') {
                                        ...
                                        $badgeClass = 'doc';
                                    } elseif (in_array($doc->file_type, ['jpg', 'jpeg', 'png', 'gif', 'webp'])) {
                                        $badgeClass = 'img';
                                    }
                                @endphp
                                <span
                                    class="doc-file-type-badge {{ $badgeClass }}">{{ strtoupper($doc->file_type ?? 'file') }}</span>
                            </div>
                            ...
                            <div class="doc-details">
                                ...
                                @if (isset($doc->rating) && $doc->rating > 0)
                                    <div class="doc-rating-stars">
                                        ...
                                        <span class="rating-count"
                                            style="font-size:0.63rem; color:#94a3b8; font-weight:500; margin-left:4px;">{{ number_format($doc->rating, 1) }}</span>
                                    </div>
                                @endif
                                ...
                                <div class="doc-meta-grid">
                                    <div class="meta-item full-width">
                                        ...
                                        <span
                                            title="{{ $doc->user->name ?? 'Unknown' }}">{{ $doc->user->name ?? 'Unknown User' }}</span>
                                    </div>
                                ...
                                <div class="doc-actions">
                                    <a href="{{ route('documents.show', $doc) }}" class="btn-doc btn-doc-view">
                                        <i
                                            class="fas fa-{{ $doc->file_type == 'pdf' ? 'book-open-reader' : 'eye' }}"></i>
                    ...
                    @empty
                        <div class="empty-state">
                            ...
                            @auth
                                <button onclick="openModal('record')" class="btn-primary"
                                    style="margin-top: 1.25rem; display: inline-flex;"><i class="fas fa-plus"></i> Upload
                                    First Record</button>
                            @endauth
                        </div>
                    @endforelse
    ...
    <div id="modalPanel" class="modal-panel">
        <form action="{{ route('documents.store') }}" method="POST" enctype="multipart/form-data" id="uploadForm">
            ...
            <div class="modal-header">
                <h2 id="modalTitle">
                    <span
                        class="w-8 h-8 bg-teal-50 rounded-lg flex items-center justify-center text-teal-600 text-sm"><i
                            class="fas fa-cloud-arrow-up"></i></span>
                    Upload Record
                </h2>
                <button type="button" onclick="closeModal()" class="modal-close"><i
            ...
            <div class="modal-body">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="form-group">
                        ...
                        <input type="text" name="author_creator" class="form-input"
                ...
                <div class="form-group">
                    ...
                    <div id="dropZone" class="drop-zone">
                        ...
                        <div id="uploadPlaceholder">
                            ...
                            <p class="text-sm text-gray-600 font-medium">Drag & drop or <span
                                    class="text-teal-600 font-semibold">browse</span></p>
                ...
                <!-- Cover Image Section -->
                <!-- EXACT COVER SECTION HTML -->
<div class="cover-section">
    <div class="cover-section-header">
        <label class="form-label"><i class="fas fa-image"></i> Cover Image</label>
        <span class="cover-hint">Optional - Auto-extracted from 1st page</span>
    </div>
    
    <div class="cover-drop-zone">
        <input type="file" name="cover_image" accept="image/*">
        <div class="cover-placeholder-text" id="coverPlaceholder">
            <i class="fas fa-cloud-arrow-up"></i>
            Drag & drop custom cover or click to browse
        </div>
    </div>

    <div class="cover-loading" id="coverLoading">
        <div class="spinner"></div>
        <span>Extracting first page...</span>
    </div>

    <div class="cover-preview-container" id="coverPreviewContainer">
        <img src="" class="cover-preview-img" id="coverPreviewImg" alt="Cover Preview">
        <button type="button" class="cover-preview-remove" id="coverRemoveBtn"><i class="fas fa-times"></i></button>
        <div class="cover-auto-badge" id="coverAutoBadge" style="display:none;"><i class="fas fa-magic"></i> Auto-extracted from PDF</div>
        <div class="cover-manual-badge" id="coverManualBadge" style="display:none;"><i class="fas fa-hand-pointer"></i> Custom cover selected</div>
    </div>
    
    <!-- Hidden input to pass base64 to Laravel -->
    <input type="hidden" name="cover_base64" id="coverBase64Input" value="">
</div>
    ...
    <div id="catModalPanel" class="cat-modal-panel">
        <div class="cat-modal-header">
            ...
            <button type="button" onclick="closeCategoryModal()" class="modal-close"><i
                    class="fas fa-xmark"></i></button>
        </div>

        <div class="cat-modal-body">
            <div class="form-group" style="margin-bottom:0;">
                <label class="form-label" for="newCategoryName">Category Name <span
                        style="color:#ef4444;">*</span></label>
                <input type="text" id="newCategoryName" class="cat-form-input"
                    placeholder="e.g. Historical Archives, Research Papers..." autocomplete="off" maxlength="100">
    ...
    <script>
        ...
        if (catNameInput) {
            catNameInput.addEventListener('input', function() {
                ...
                document.getElementById('catCharCount').querySelector('span').textContent = len +
                    ' / 100 characters';
                ...
                .then(response => {
                    if (!response.ok) {
                        return response.json().then(data => {
                            throw new Error(data.message || data.errors?.name?.[0] ||
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        console.log("Initializing PDF Cover Preview...");

        // Get elements by ID to be 100% sure we find them
        const mainFileInput = document.querySelector('input[name="file"]');
        const manualCoverInput = document.querySelector('input[name="cover_image"]');
        const coverBase64Input = document.getElementById('coverBase64Input');
        const coverPreviewContainer = document.getElementById('coverPreviewContainer');
        const coverPreviewImg = document.getElementById('coverPreviewImg');
        const coverLoading = document.getElementById('coverLoading');
        const coverPlaceholder = document.getElementById('coverPlaceholder');
        const coverAutoBadge = document.getElementById('coverAutoBadge');
        const coverManualBadge = document.getElementById('coverManualBadge');
        const removeBtn = document.getElementById('coverRemoveBtn');

        // Safety checks
        if (!mainFileInput) { console.error("Missing: input[name='file']"); return; }
        if (!coverPreviewContainer) { console.error("Missing: #coverPreviewContainer. Did you add the HTML block?"); return; }
        if (!coverBase64Input) { console.error("Missing: #coverBase64Input"); return; }

        function resetCover() {
            coverPreviewContainer.style.display = 'none';
            coverLoading.style.display = 'none';
            if(coverAutoBadge) coverAutoBadge.style.display = 'none';
            if(coverManualBadge) coverManualBadge.style.display = 'none';
            if(coverPlaceholder) coverPlaceholder.style.display = 'block';
            coverBase64Input.value = '';
            if(manualCoverInput) manualCoverInput.value = '';
        }

        function showCover(base64String, isAuto) {
            coverLoading.style.display = 'none';
            if(coverPlaceholder) coverPlaceholder.style.display = 'none';
            
            coverPreviewImg.src = base64String;
            coverPreviewContainer.style.display = 'block';
            
            if (isAuto && coverAutoBadge) {
                coverAutoBadge.style.display = 'inline-flex';
                if(coverManualBadge) coverManualBadge.style.display = 'none';
            } else if (!isAuto && coverManualBadge) {
                coverManualBadge.style.display = 'inline-flex';
                if(coverAutoBadge) coverAutoBadge.style.display = 'none';
            }

            // Save to hidden input so Laravel receives it
            coverBase64Input.value = base64String;
            console.log("Cover base64 saved to input successfully.");
        }

        // 1. When main PDF is selected
        ...
        // 2. When user manually picks a cover image
        if (manualCoverInput) {
            manualCoverInput.addEventListener('change', function (e) {
                const file = e.target.files[0];
                if (!file) return;
                ...
                const reader = new FileReader();
                reader.onload = function (ev) {
                    showCover(ev.target.result, false); 
                };
                reader.readAsDataURL(file);
            });
        }
