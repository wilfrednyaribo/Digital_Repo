    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        @supports not (aspect-ratio: 3 / 4.2) {
            .r-book-cover { height: 0; padding-bottom: 140%; position: relative; }
            .r-book-cover > * { position: absolute !important; inset: 0; width: 100% !important; height: 100% !important; }
        }
        ...
        .r-file-badge { display: inline-flex; align-items: center; justify-content: center; padding: 1px 6px; border-radius: 3px; font-size: 0.5rem; font-weight: 800; letter-spacing: 0.04em; text-transform: uppercase; line-height: 1.6; flex-shrink: 0; }
        ...
        /* ╔═══════════════════════════════════════════════╗
           ...
           ╚═══════════════════════════════════════════════╝ */
        .r-overlay { position: fixed; inset: 0; z-index: 200; display: none; flex-direction: column; background: #FEFDFB; transition: background 0.3s; }
        .r-overlay.open { display: flex; }
        .r-overlay.dark-mode { background: var(--gok-black); }
        .r-overlay.sepia-mode { background: #F5F0E3; }
        .r-reader-bar { height: 52px; background: var(--gok-white); border-bottom: 2px solid var(--gok-green); display: flex; align-items: center; justify-content: space-between; padding: 0 1.25rem; flex-shrink: 0; transition: background 0.3s, border-color 0.3s; }
        ...
        .r-reader-back { background: none; border: none; color: var(--gok-gray-500); cursor: pointer; font-size: 0.76rem; font-weight: 600; font-family: 'Inter', sans-serif; display: flex; align-items: center; gap: 6px; transition: color 0.2s; padding: 0; }
        .r-reader-back:hover { color: var(--gok-green); }
        .r-reader-title { font-size: 0.8rem; font-weight: 600; color: var(--gok-gray-600); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 320px; transition: color 0.3s; }
        ...
        .r-ctrl-btn { width: 34px; height: 34px; border-radius: 8px; border: 1px solid var(--gok-gray-200); background: white; color: var(--gok-gray-500); cursor: pointer; display: flex; align-items: center; justify-content: center; font-size: 0.75rem; transition: all 0.2s; flex-shrink: 0; }
        ...
        .r-font-select { background: white; border: 1px solid var(--gok-gray-200); border-radius: 8px; color: var(--gok-gray-600); font-size: 0.7rem; padding: 6px 8px; font-family: 'Inter', sans-serif; cursor: pointer; outline: none; max-width: 110px; }
        ...
        .r-zoom-widget { display: flex; align-items: center; gap: 0; background: var(--gok-gray-50); border-radius: 8px; border: 1px solid var(--gok-gray-200); overflow: hidden; flex-shrink: 0; transition: background 0.3s, border-color 0.3s; }
        .dark-mode .r-zoom-widget { background: #1a1a1a; border-color: #333; }
        .sepia-mode .r-zoom-widget { background: #E5DCC8; border-color: #C9BDAA; }
        .r-zoom-btn { width: 34px; height: 34px; border: none; background: transparent; color: var(--gok-gray-500); cursor: pointer; display: flex; align-items: center; justify-content: center; font-size: 0.78rem; transition: all 0.15s; flex-shrink: 0; }
        ...
        .r-zoom-value { min-width: 46px; text-align: center; font-size: 0.72rem; font-weight: 700; color: var(--gok-gray-700); font-family: 'Inter', sans-serif; border-left: 1px solid var(--gok-gray-200); border-right: 1px solid var(--gok-gray-200); padding: 0 4px; line-height: 34px; cursor: pointer; position: relative; transition: color 0.3s, border-color 0.3s; }
        .dark-mode .r-zoom-value { color: #ccc; border-color: #333; }
        .sepia-mode .r-zoom-value { color: #5C4D3A; border-color: #C9BDAA; }
        .r-zoom-dropdown { position: absolute; top: 100%; left: 50%; transform: translateX(-50%); background: white; border: 1px solid var(--gok-gray-200); border-radius: 10px; box-shadow: 0 8px 30px rgba(0,0,0,0.12); padding: 4px; min-width: 80px; z-index: 20; display: none; margin-top: 4px; }
        ...
        .r-zoom-option { display: block; width: 100%; text-align: center; padding: 6px 12px; border: none; background: transparent; color: var(--gok-gray-600); font-size: 0.72rem; font-weight: 500; cursor: pointer; border-radius: 6px; font-family: 'Inter', sans-serif; transition: all 0.1s; }
        ...
        .r-reader-page { position: absolute; inset: 0; overflow-y: auto; padding: 2.5rem 0; scrollbar-width: thin; scrollbar-color: var(--gok-gray-200) transparent; transition: background 0.3s; }
        ...
        .r-chapter-title { font-family: 'Playfair Display', serif; font-size: 2rem; font-weight: 700; line-height: 1.25; margin-bottom: 0.5rem; letter-spacing: -0.01em; transition: color 0.3s; }
        .dark-mode .r-chapter-title { color: var(--gok-white); }
        .sepia-mode .r-chapter-title { color: #2C2218; }
        .r-chapter-sub { font-size: 0.85rem; font-weight: 400; margin-bottom: 2.5rem; padding-bottom: 2rem; border-bottom: 1px solid var(--gok-gray-200); transition: color 0.3s, border-color 0.3s; }
        ...
        .r-reader-text p:first-child::first-letter { font-size: 3.2em; float: left; line-height: 0.8; margin-right: 0.08em; font-weight: 700; color: var(--gok-green); font-family: 'Playfair Display', serif; transition: color 0.3s; }
        ...
        .r-reader-footer { height: 40px; background: var(--gok-white); border-top: 2px solid var(--gok-green); display: flex; align-items: center; justify-content: space-between; padding: 0 1.5rem; flex-shrink: 0; transition: background 0.3s, border-color 0.3s; }
        ...
        .pdf-page-canvas { display: block; width: 100%; max-width: 680px; margin: 0 auto 1.5rem; border-radius: 4px; box-shadow: 0 2px 16px rgba(0,0,0,0.08); background: white; transition: transform 0.25s cubic-bezier(0.4, 0, 0.2, 1); transform-origin: top center; }
        ...
        @media (max-width: 1200px) { .r-grid { grid-template-columns: repeat(4, 1fr); gap: 1.1rem; align-items: stretch; } }
        @media (max-width: 900px) { .r-grid { grid-template-columns: repeat(3, 1fr); gap: 1rem; align-items: stretch; } }
        @media (max-width: 768px) {
            ...
            .r-grid { grid-template-columns: repeat(2, 1fr); gap: 0.85rem; align-items: stretch; }
...
<script>
    ...
    function showLoading(msg) { document.getElementById('readerContent').innerHTML = '<div class="r-reader-state"><div style="width:40px;height:40px;border:3px solid var(--gok-gray-200);border-top-color:var(--gok-green);border-radius:50%;animation:spin 0.8s linear infinite;"></div><p style="color:var(--gok-gray-400);font-size:0.85rem;margin:0;">' + (msg || 'Loading...') + '</p></div>'; }
    function showError(title, msg) { document.getElementById('readerContent').innerHTML = '<div class="r-reader-state"><div class="r-reader-state-icon"><i class="fas fa-exclamation-triangle" style="color:var(--gok-red);"></i></div><h2>' + title + '</h2><p>' + msg + '</p><button class="r-reader-state-btn" onclick="closeReader()"><i class="fas fa-arrow-left" style="font-size:0.75rem;"></i> Go Back</button></div>'; }

    function zoomIn() { setZoom(Math.min(300, zoomLevel + ZOOM_STEP)); }
    function zoomOut() { setZoom(Math.max(50, zoomLevel - ZOOM_STEP)); }
    function setZoom(level) { zoomLevel = level; updateZoomUI(); applyZoom(); }
    function updateZoomUI() {
        ...
        document.querySelectorAll('.r-zoom-option').forEach(opt => { opt.classList.toggle('active', parseInt(opt.dataset.zoom) === zoomLevel); });
    }
    function applyZoom() {
        const scale = zoomLevel / 100;
        if (currentReaderMode === 'pdf') {
            document.querySelectorAll('.pdf-page-canvas').forEach(canvas => { canvas.style.transform = 'scale(' + scale + ')'; canvas.style.marginBottom = Math.max(8, 24 * scale) + 'px'; });
            const inner = document.querySelector('.r-page-inner');
            if (inner) inner.style.maxWidth = (700 * scale) + 'px';
        } else {
            const newSize = Math.max(10, Math.min(48, Math.round(18 * scale)));
            currentFontSize = newSize;
            const textEl = document.querySelector('.r-reader-text');
            if (textEl) textEl.style.fontSize = newSize + 'px';
        }
    }
    function toggleZoomDropdown() { document.getElementById('zoomDropdown').classList.toggle('open'); }
    function buildZoomDropdown() {
        document.getElementById('zoomDropdown').innerHTML = ZOOM_LEVELS.map(z => '<button class="r-zoom-option' + (z === zoomLevel ? ' active' : '') + '" data-zoom="' + z + '" onclick="setZoom(' + z + '); toggleZoomDropdown();">' + z + '%</button>').join('');
    }
    document.addEventListener('click', e => {
        const dd = document.getElementById('zoomDropdown');
        const val = document.getElementById('zoomValue');
        if (dd.classList.contains('open') && !val.contains(e.target)) dd.classList.remove('open');
    });
